...
def main(args):
    ...
    if args.do_test:
        ...
        # y_test_np is the ground-truth "target" for the test split
        # raw_test_pred is the model's raw prediction of that target
        truth_pred_df = pd.DataFrame({
            "truth": y_test_np,               # ground-truth target (market_forward_excess_returns)
            "prediction": raw_test_pred       # model's predicted target
        })

        truth_pred_df.to_csv(f"test_truth_prediction{args.model}.csv", index=False)
        logging.info("Saved test truth/prediction CSV -> test_truth_prediction.csv")
